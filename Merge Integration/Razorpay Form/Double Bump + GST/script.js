...
const elements = {
  ...
  gstValue: document.getElementsByClassName("gst")[0],
...
const calculateTotalWithGST = (baseAmount) => {
  const gstAmount = Number(baseAmount) * config.GST_RATE;
  return Number(baseAmount) + gstAmount;
};

const updateDisplayValues = () => {
  const baseAmount = calculateTotalAmount();
  const gstAmount = baseAmount * config.GST_RATE;
  const total = calculateTotalWithGST(baseAmount);

  elements.amountValue.innerText = formatCurrency(baseAmount);
  elements.gstValue.innerText = formatCurrency(gstAmount);
  elements.totalAmountValue.innerText = formatCurrency(total);
  elements.paymentButton.innerText = `Pay ${formatCurrency(total)}`;
...
const getRazorpayOptions = (orderId) => ({
  key: config.apiKey,
  amount: calculateTotalWithGST(calculateTotalAmount()) * 100,
  currency: "INR",
...
const setButtonState = (disabled, text) => {
  ...
  elements.paymentButton.innerText =
    text ||
    `Pay ${formatCurrency(calculateTotalWithGST(calculateTotalAmount()))}`;
};
